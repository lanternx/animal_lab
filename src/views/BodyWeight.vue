...
setup() {
    ...
    try {
        ...
        // 按笼位排序
        lived_mice.value.sort((a, b) => {
        if (a.cage_id === -1 && b.cage_id === -1) return 0;
        if (a.cage_id === -1) return 1;
        if (b.cage_id === -1) return -1;
        return parseInt(a.cage_id) - parseInt(b.cage_id);
        });
    ...
    try {
        ...
        const datasets = [];
        const colors = ['#6a11cb', '#2575fc', '#20c997', '#ff416c', '#ff4b2b'];
