...
setup(props, { emit }) {
    ...
    try {
        const response = await fetch(`/api/mice/${props.mouseId}`)
    ...
    try {
        const recordToSave = { ...newRecord.value, mouse_tid: props.mouseId, birth_date: mouseData.value.birth_date }
...
const renderPedigreeChart = async () => {
  ...
  simulationRef.value = simulation
    ...
    const navigateToMouse = (mouseId) => {
        sessionStorage.setItem('prevMouseId', props.mouseId);
        router.push(`/mouse/${mouseId}`);
    };

    onMounted(() => {
    fetchMouseData()
    setupResizeObserver()
    const storedId = sessionStorage.getItem('prevMouseId');
    if (storedId && storedId !== props.mouseId) {
        prevMouseId.value = storedId;
    } else {
    prevMouseId.value = null;
    }
    })
